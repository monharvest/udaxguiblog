...
const PostDetailPage = ({ post, onBack }) => {
    ...
    const handleReadBookClick = () => {
        // FOR TESTING: Always use the local PDF
        const testPdfUrl = "/Үндэстнүүдэд_зориулсан_сайн_мэдээ_2_compressed.pdf";

        if (testPdfUrl && window.jQuery && typeof window.jQuery.fn.df_flipbook === 'function') {
            console.log("Attempting to open flipbook with local test PDF...");
            const options = {
                source: testPdfUrl,
                webgl: true,
                hard: 'cover'
            ...
            window.jQuery(document.body).df_flipbook(options);
             ...
             console.error("Flipbook library is not ready. Check script loading in layout.js and jQuery availability.");
             alert("The flipbook library is not ready. Please wait a moment or refresh the page.");
        }
    };
...
const PostForm = ({ initialPost, onSubmit, isSubmitting, buttonText }) => {
    ...
    const [pdfUrl, setPdfUrl] = useState(initialPost?.pdfUrl || '');
    ...
    const uploadFile = async (file, type) => {
        ...
        try {
            const response = await fetch(`${WORKER_URL}/api/${type}s/upload`, {
                method: 'POST',
    ...
    const handleSubmit = async (e) => {
        ...
        if (!finalImageUrl || !finalPdfUrl) {
            alert('Зураг болон PDF файлыг заавал сонгоно уу.');
            return;
        }

        onSubmit({ ...initialPost, title, excerpt, imageUrl: finalImageUrl, category, content, metaDescription, pdfUrl: finalPdfUrl });
    };
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4"><label className="block text-gray-700 font-semibold mb-2">Нийтлэлийн зураг</label><input type="file" accept="image/*" onChange={(e) => handleFileChange(e, 'image')} className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"/>{imagePreview && <img src={imagePreview} alt="Preview" className="mt-4 rounded-lg max-h-40" />}</div>
            <div className="mb-4"><label className="block text-gray-700 font-semibold mb-2">PDF Файл</label><input type="file" accept=".pdf" onChange={(e) => handleFileChange(e, 'pdf')} className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"/>{pdfFile && <p className="text-sm text-gray-500 mt-2">Selected: {pdfFile.name}</p>}{pdfUrl && !pdfFile && <p className="text-sm text-gray-500 mt-2">Current PDF: <a href={pdfUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600">View</a></p>}</div>
